...
SELECT
  ...
  'pending',
  COALESCE(l.week1_sent_at, l.payment_date, NOW()) + v.delay_interval,
  v.metadata
FROM leads l
JOIN (
  VALUES
...
) AS v(template_key, delay_interval, metadata) ON TRUE
WHERE l.status = 'paid'
  AND l.payment_date IS NOT NULL
  AND (v.template_key NOT IN ('week_2', 'week_3', 'week_4') OR l.week1_sent_at IS NOT NULL)
